...
from app.modules.partners.mapper import public_type_details
from app.modules.partners.schemas import PartnerDraftIn
from app.modules.planner.domain import build_planner_partner_recommendations
from scripts.audit_culinary_partners import matched_terms
...
def culinary_draft(**overrides):
    ...
    return PartnerDraftIn.model_validate(values)
...
def test_culinary_fields_are_normalized_without_price_fields():
    ...
    changes = partner_draft_changes(payload, "now")
...
def test_culinary_submission_requires_signature_food_or_drinks():
    with pytest.raises(PartnerError) as error:
        validate_submission(submission_doc(culinary_specialties=[]))
    assert "culinary_specialties" in error.value.detail["fields"]

    validate_submission(submission_doc())
...
def test_culinary_completeness_and_public_type_details_are_safe():
    ...
    score, missing = partner_completeness(complete, offerings_count=1)
    assert score == 100
    assert missing == []

    complete["owner_user_id"] = "private-owner"
    complete["verification_documents"] = [{"filename": "private.pdf"}]
    details = public_type_details(complete)
...
def test_planner_matches_culinary_keywords_and_real_profile_tags():
    ...
    assert any(
        "kopi lintong" in reason for reason in recommendations[0]["match_reasons"]
    )


@pytest.mark.parametrize(
    "keyword", ["kuliner", "makanan", "kopi", "restaurant", "food"]
)
