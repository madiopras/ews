...
from app.modules.partners.schemas import PartnerAnalyticsEventIn
from app.modules.planner.domain import (
    PLANNER_PARTNER_RELEVANCE_THRESHOLD,
...
def test_recommendation_exposes_auditable_relevance_without_premium_ranking_boost():
    ...
    _, recommendations = build_planner_partner_recommendations(
        "Danau Toba",
        [destination],
        {destination_id: candidates},
        [],
        "butuh pemandu keluarga",
        "id",
    )
    ...
    assert all(
        row["relevance_score"] >= PLANNER_PARTNER_RELEVANCE_THRESHOLD
        for row in recommendations
    )
    assert all(
        set(row["match_factor_codes"])
        == {"destination_coverage", "requested_service_type", "service_tag_match"}
        for row in recommendations
    )
...
def test_health_monitoring_raises_alert_only_after_minimum_sample():
    ...
    health = planner_health_summary(logs)
    ...
    assert planner_health_summary(logs[:5])["alert"]["active"] is False
...
def test_fairness_summary_audits_segments_equal_scores_images_and_gaps():
    ...
    analytics = [
        {
            ...
            "partner_id": first_id if index < 4 else second_id,
            "partner_type": "guide",
            "tier": "regular",
            "relevance_score": 70,
        }
    ...
    logs = [
        {
            "status": "completed",
            "partner_gap_keys": ["Toba|homestay", "Toba|homestay", "Samosir|rental"],
        }
    ]

    fairness = governance_fairness_summary(analytics, exposure, partners, logs)
    ...
    assert fairness["empty_results_by_area_type"][0] == {
        "area": "Toba",
        "type": "homestay",
        "empty_results": 2,
    }
