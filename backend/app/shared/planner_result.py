...
PlannerIdentifier = Annotated[
    str, StringConstraints(strip_whitespace=True, min_length=1, max_length=64)
]
PlannerTag = Annotated[
    str, StringConstraints(strip_whitespace=True, min_length=1, max_length=50)
]
PlannerImage = Annotated[
    str, StringConstraints(strip_whitespace=True, min_length=1, max_length=1000)
]
PlannerReason = Annotated[
    str, StringConstraints(strip_whitespace=True, min_length=1, max_length=240)
]
PlannerNote = Annotated[
...
class PlannerPartnerMatchOut(PlannerContractModel):
    ...
    type: PlannerPartnerType
    destination_ids: List[PlannerIdentifier] = Field(
...
class PlannerStoredResultV2(PlannerContractModel):
    ...
    partner_matches: List[PlannerStoredPartnerMatch] = Field(
    ...
    def validate_stored_relationships(self):
        ...
        if any(
            ...
            raise ValueError(
                "every stop destination must be declared in destination_ids"
            )
...
class PlannerResultV2(PlannerContractModel):
    ...
    destinations: List[PlannerDestinationCardOut] = Field(
        default_factory=list, max_length=50
    )
    partner_matches: List[PlannerPartnerMatchOut] = Field(
        default_factory=list, max_length=20
    )
    travel_notes: List[PlannerNote] = Field(default_factory=list, max_length=10)
    travel_tips: List[PlannerNote] = Field(default_factory=list, max_length=10)
    generated_at: str = Field(
        default_factory=lambda: datetime.now(timezone.utc).isoformat(), max_length=40
    )
    ...
    def validate_result_relationships(self):
        ...
        used_ids = {stop.destination_id for day in self.days for stop in day.stops}
...
def planner_partner_public_from_doc(
    ...
    *,
    ...
    if doc.get("status") != "approved" or doc.get("is_active", True) is False:
        raise ValueError(
            "only active approved partners can be exposed in Planner results"
        )
    ...
    return PlannerPartnerPublicOut(
        ...
        service_tags=[
            str(value)[:50] for value in (doc.get("service_tags") or []) if value
        ][:20],
        is_premium=premium,
