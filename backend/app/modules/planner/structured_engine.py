...
def select_structured_catalog(
    ...
    *,
    ...
    interest_terms = {
        str(value).replace("_", " ").lower() for value in interests if value
    }
    context_terms = {
        token
        for token in re.findall(r"[a-z0-9-]{3,}", extra_context.lower())
        if token
        not in {"yang", "untuk", "dengan", "saya", "ingin", "the", "and", "for", "with"}
    }
    ...
    def score(row: dict) -> tuple[int, int, int, str]:
        ...
        interest_score = sum(
            term == category or term in tags for term in interest_terms
        )
...
def hydrate_structured_planner_result(
    ...
    *,
    ...
    destination_ids = list(
        dict.fromkeys(
            stop.destination_id for day in provider_result.days for stop in day.stops
        )
    )
    documents = {
    ...
    for item in partner_matches:
        try:
            matches.append(
                PlannerPartnerMatchOut.model_validate(
                    {
                        key: value
                        for key, value in item.items()
                        if key in allowed_match_fields
                    }
                )
            )
...
def structured_result_to_markdown(
    result: PlannerResultV2, lang: Literal["id", "en"]
) -> str:
    """Deterministic compatibility view used until every client renders V2 cards."""
    destination_map = {
        destination.id: destination for destination in result.destinations
    }
    lines = [result.summary]
    for day in result.days:
        lines.extend(
            ["", f"## {'Hari' if lang == 'id' else 'Day'} {day.day}: {day.title}"]
        )
        if day.area_label:
        ...
        for stop in day.stops:
            ...
            name = (
                destination.name_en
                if lang == "en" and destination.name_en
                else destination.name
            )
            details = " · ".join(
                value for value in (stop.time_label, destination.location) if value
            )
            lines.extend(
                ["", f"### {name}", f"*{details}*" if details else "", stop.activity]
            )
    ...
    if result.travel_notes:
        lines.extend(
            ["", "### Catatan Perjalanan" if lang == "id" else "### Trip Notes"]
        )
...
def normalize_legacy_fallback(raw: str) -> str:
    ...
    text = raw.strip().lstrip("\ufeff")
    fenced = re.fullmatch(
        r"```(?:json|markdown|md)?\s*(.*?)\s*```", text, flags=re.IGNORECASE | re.DOTALL
    )
