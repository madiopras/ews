    ...
    for line in lines:
        ...
        for match in matches:
            processed_line = processed_line.replace(
                f'ObjectId("{match}")', f'"__OBJECTID__{match}__"'
            )
        ...
        for match in matches:
            processed_line = processed_line.replace(
                f'ISODate("{match}")', f'"__ISODATE__{match}__"'
            )
...
def import_destinations():
    ...
    try:
        ...
        # Read JSON file
        json_file = PROJECT_DIR / "docsplan" / "destinations.json"
        ...
        for dest in destinations:
            doc = {
                ...
                "tags": dest.get("tags", []),
                "source_label": dest.get(
                    "source_label", "Instagram @explorewisatasumut"
                ),
                "source_url": dest.get(
                    "source_url", "https://www.instagram.com/explorewisatasumut/"
                ),
                "editorial_reviewed_at": dest.get(
                    "editorial_reviewed_at", dest["created_at"]
                ),
