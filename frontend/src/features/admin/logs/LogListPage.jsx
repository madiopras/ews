...
export default function LogListPage({ type }) {
  ...
  const columns = useMemo(() => {
    ...
    if (type === "ai") return [
      { key: "created_at", header: copy.time, render: (i) => formatDate(i.created_at) }, { key: "status", header: copy.status, render: (i) => statusBadge(i.status) }, { key: "request", header: copy.request, render: (i) => <div>{i.days} {copy.days} · IDR {Number(i.budget || 0).toLocaleString(locale)}<div className="text-[10px] text-inkSoft">{i.lang?.toUpperCase()} · {i.catalog_size} {copy.catalogShort}</div></div> }, { key: "model", header: copy.model, render: (i) => <div><code className="text-xs">{i.llm_model || "-"}</code><div className="text-[10px] text-inkSoft">{i.llm_profile_name || i.llm_source || "environment"}</div></div> }, { key: "duration", header: copy.duration, render: (i) => i.duration_ms != null ? `${i.duration_ms} ms` : "-" }, { key: "view", header: "", mobile: false, render: (i) => <button type="button" aria-label={copy.viewDetails} className="w-9 h-9 rounded-lg hover:bg-line/30 inline-flex items-center justify-center" onClick={() => setDetail(i)}><Eye className="w-4 h-4" /></button> },
    ];
    return [
